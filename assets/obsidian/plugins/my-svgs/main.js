...
module.exports = class SvgIconsPlugin extends Plugin {
  ...
  async onload() {
    ...
    if (this.settings.syncUsedOnly && this.settings.iconsFolder) {
      await this.syncUsedIcons();
    }
  ...
  getIconsPaths() {
    if (Platform.isMobile && this.settings.iconsFolder) {
      return [normalizePath(this.settings.iconsFolder)];
    }
    return [normalizePath(`${this.manifest.dir}/icons`)];
  }

  async getUsedIconNames() {
    const iconicDataPath = normalizePath(".obsidian/plugins/iconic/data.json");
    if (!(await this.app.vault.adapter.exists(iconicDataPath))) {
      return new Set();
    }

    try {
      const raw = await this.app.vault.adapter.read(iconicDataPath);
      const data = JSON.parse(raw);
      const prefix = this.settings.iconPrefix;
      const used = new Set();

      const objSections = [
        "fileIcons", "bookmarkIcons", "tagIcons",
        "propertyIcons", "ribbonIcons", "tabIcons", "appIcons",
      ];
      for (const section of objSections) {
        if (data[section] && typeof data[section] === "object") {
          for (const entry of Object.values(data[section])) {
            if (entry.icon && entry.icon.startsWith(prefix)) {
              used.add(entry.icon.slice(prefix.length));
            }
          }
        }
      }

      const arrSections = ["fileRules", "folderRules"];
      for (const section of arrSections) {
        if (Array.isArray(data[section])) {
          for (const rule of data[section]) {
            if (rule.icon && rule.icon.startsWith(prefix)) {
              used.add(rule.icon.slice(prefix.length));
            }
          }
        }
      }
      ...
      console.error("My SVGs: Failed to parse Iconic data.json:", error);
      return new Set();
    }
  }

  async syncUsedIcons() {
    const usedNames = await this.getUsedIconNames();
    if (usedNames.size === 0) return;

    const vaultFolder = this.settings.iconsFolder
      ? normalizePath(this.settings.iconsFolder)
      : null;
    if (!vaultFolder) return;

    if (!(await this.app.vault.adapter.exists(vaultFolder))) {
      await this.app.vault.adapter.mkdir(vaultFolder);
    }

    const currentFiles = await this.app.vault.adapter.list(vaultFolder);
    const currentSvgs = new Set(
      currentFiles.files
        .filter((f) => f.endsWith(".svg"))
        .map((f) => f.split("/").pop().replace(".svg", "")),
    );

    const toAdd = [...usedNames].filter((name) => !currentSvgs.has(name));
    const toRemove = [...currentSvgs].filter((name) => !usedNames.has(name));

    if (Platform.isDesktop && toAdd.length > 0) {
      const sourcePath = normalizePath(`${this.manifest.dir}/icons`);
      if (await this.app.vault.adapter.exists(sourcePath)) {
        for (const name of toAdd) {
          const srcFile = normalizePath(`${sourcePath}/${name}.svg`);
          const dstFile = normalizePath(`${vaultFolder}/${name}.svg`);
          try {
            if (await this.app.vault.adapter.exists(srcFile)) {
              const content = await this.app.vault.adapter.read(srcFile);
              await this.app.vault.adapter.write(dstFile, content);
            }
          } catch (err) {
            console.error(`My SVGs: Failed to copy ${name}.svg:`, err);
          }
        }
      }
    }
    ...
    if (toRemove.length > 0) {
      for (const name of toRemove) {
        const filePath = normalizePath(`${vaultFolder}/${name}.svg`);
        try {
          await this.app.vault.adapter.remove(filePath);
        } catch (err) {
          console.error(`My SVGs: Failed to remove ${name}.svg:`, err);
        }
      }
    }
    ...
    if (toAdd.length > 0 || toRemove.length > 0) {
      console.log(
        `My SVGs: Sync complete. Added: ${toAdd.length}, Removed: ${toRemove.length}, Used: ${usedNames.size}`,
      );
    }
  }
  ...
  async loadPluginIcons() {
    try {
      let iconsPath = null;
      for (const candidate of this.getIconsPaths()) {
      ...
      if (!iconsPath) {
        console.log("My SVGs: no icons directory found at any configured path");
        return;
      }
      ...
      console.log(`My SVGs: loaded ${svgFiles.length} icons from ${iconsPath}`);
    } catch (error) {
...
class SvgIconsSettingTab extends PluginSettingTab {
  ...
  display() {
    ...
    new Setting(containerEl)
      .setName("Icons folder (vault path)")
      .setDesc(
        "Path to a folder inside your vault for SVG icons (e.g., 'Файлы/icons'). Syncs across all devices including iOS. Falls back to plugin folder if empty.",
      )
      .addText((text) =>
        text
          .setPlaceholder("e.g. Файлы/icons")
    ...
    new Setting(containerEl)
      .setName("Sync only used icons")
      .setDesc(
        "Only keep icons referenced by Iconic in the vault folder. Reduces sync to iOS from thousands of files to just the ones you actually use.",
      )
      .addToggle((toggle) =>
        toggle
          .setValue(this.plugin.settings.syncUsedOnly)
          .onChange(async (value) => {
            this.plugin.settings.syncUsedOnly = value;
            await this.plugin.saveSettings();
          }),
      );

    new Setting(containerEl)
      .setName("Sync used icons now")
      .setDesc(
        "Scan Iconic config, copy used icons to vault folder, remove unused ones.",
      )
      .addButton((button) =>
        button
          .setButtonText("Sync now")
          .onClick(async () => {
            await this.plugin.syncUsedIcons();
            new Notice("Icon sync complete!");
  ...
  async displayIconsGrid(container) {
    try {
      let iconsPath = null;
      for (const candidate of this.plugin.getIconsPaths()) {
        if (await this.app.vault.adapter.exists(candidate)) {
          iconsPath = candidate;
          break;
        }
      ...
      if (!iconsPath) {
        ...
        errorMsg.createEl("p", {
          text: "Icons directory not found.",
        });
        errorMsg.createEl("p", {
          text: "Set a vault folder path in settings or create .obsidian/plugins/my-svgs/icons/",
        });
        return;
      }
